...
these six fields **on each module** with the exact API names and types below:
...
| True Colors Analysis URL | URL  | `True_Colors_Analysis_URL`    |
...
- `Contact` — lookup to the standard Contacts module (we pull `First_Name` / `Last_Name` from there)

> If your CRM admin uses different API names for these fields, change them once
> in `lib/zoho.ts` (`ZOHO_FIELDS` constant) instead of renaming in Zoho.
...
2. Writes the five score/date fields to that `Contacts_Hub` record.
3. **If `Relationship == "Opportunity Owner"` AND the `Deal` lookup is
   populated**, mirrors the same fields onto the linked Deal.
4. Runs the analysis pipeline (Anthropic → PDF → Supabase Storage), then
   writes the resulting signed URL back to `True_Colors_Analysis_URL` on both
   records (same Opportunity-Owner gate).

If either **score** write fails, the assessment is **not** marked complete,
the error is captured in `zoho_sync_error`, and the API returns 502 so the
client can retry. Re-trying re-writes Contacts_Hub harmlessly (idempotent) and
only attempts the Deal write if the condition still holds.

If the **analysis** pipeline fails, the assessment is still marked complete
(scores already in Zoho), the error is logged to `analysis_error`, and the
candidate still sees "Sent ✓". Re-mint a token and re-run if you need the PDF.
...
## 2d. Anthropic (analysis report)

The post-submit analysis report is generated by Claude (model `claude-sonnet-4-5`)
and rendered to a PDF. Add an API key from <https://console.anthropic.com> to
`.env.local` and to Vercel:

```
ANTHROPIC_API_KEY=sk-ant-...
```

The model string lives at the top of `lib/analysis.ts` (`ANALYSIS_MODEL`
constant) — bump it there to roll forward. The system prompt lives in the same
file and can be edited freely.

## 2e. Supabase Storage bucket (analysis PDFs)

Create one **private** bucket manually in Supabase Studio
(Storage → New Bucket):
...
Objects are written as `{token}.pdf` (overwritten on resubmit). The submit
route generates a **90-day signed URL** with the service role key and writes
that URL to the Zoho `True_Colors_Analysis_URL` field. Andrea opens the PDF
from inside Zoho — no public bucket exposure required.
...
candidate finishes row 6                → POST /api/submit/[token]
  ...
  → analysis pipeline (best-effort, never 502s):
      → Anthropic generates analysis JSON
      → @react-pdf renders to a Buffer
      → upload to Supabase Storage (tc-analysis/{token}.pdf), get 90-day signed URL
      → PUT URL to Contacts_Hub.True_Colors_Analysis_URL (and Deal if applicable)
      → save analysis_json + analysis_url to assessment_progress
